...
`base_label` field is always the fixed string "MLS Component". The
...
This part of the Safe Application Interface allows components to make use of the
HPKE key pairs generated by MLS. A component identified by a ComponentID can use
any HPKE key pair for any operation defined in {{!RFC9180}}, such as encryption,
exporting keys, and the PSK mode, as long as the `info` input to `Setup<MODE>S`
and `Setup<MODE>R` is set to ComponentOperationLabel with `component_id` set to
the appropriate ComponentID. The `context` can be set to an arbitrary Context
...
The safe exporter is constructed from an Exporter Tree, a tree of secrets with
the same structure as the Secret Tree defined in {{Section 9 of !RFC9420}}, with
...
is in `proposal_types`, every member of the group supports the AppDataUpdate
...
* If the list comprises one or more proposals, all with `op` field set to
  `update`:
    ...
    * Otherwise, insert a new entry in the `component_data` vector with the
      ...
      `component_data` vector sorted by `component_id`.

* Otherwise, the proposal list is invalid.

> NOTE: Putting large amounts of data into the `app_data_dictionary`
> GroupContext extension may have a performance impact.
...
To avoid storing large amounts of data in the `app_data_dictionary` GroupContext
extension, implementations MAY offer storing only a hash of the actual data in
the GroupContext extension.
...
In an MLS `PrivateMessage`, the `authenticated_data` is also present in
...
The Safe AAD framing allows multiple application components
...
> The "MLS Extension Types" registry represents extensibility of four
...
{{Section 11.1 of !RFC9420}} and reproduced below) contains all non-default
extensions that are mandatory to support in its `extension_types` vector. Its
`proposal_types` vector contains any Proposals that are mandatory to support.
...
The `safe_aad` component contains a list of component IDs. When present (in an
`app_data_dictionary` extension) in a LeafNode, the semantics are the list of
supported components that use Safe AAD. When present (in an
`app_data_dictionary` extension) in the GroupContext, the semantics are the list
...
new component `app_components`. When present in a LeafNode, the semantics are
the list of supported components. When present in the GroupContext, the
semantics are the list of required components.
...
* LeafNode.extensions.app_data_dictionary.safe_aad
* LeafNode.extensions.app_data_dictionary.app_components
...
AppEphemeral Proposal with a component ID `app_ack`, so that it is included in
...
of messages whose `generation` values form a continuous range from
...
The struct above reproduces the `Proposal` type from {{!RFC9420}} with the
`self_remove` case added to show where SelfRemove fits. It is not the complete
extended set: the `app_ephemeral` and `app_data_update` proposal types defined
in this document extend the same enum.
...
other pending Proposals when sending a Commit. It already MUST send a Commit of
...
As a result, a last_resort_key_package component contains the `component_id`
...
in the context of a given LeafNode if all of the following are true:
...
This document also requests the creation of a new "MLS Component Types"
...
- Where: The object(s) in which the component may appear,
...
| 0x0001   | app_components           | LN,GC | Y | RFC XXXX |
| 0x0002   | safe_aad                 | LN,GC | Y | RFC XXXX |
| 0x0003   | content_media_types      | LN,GC | Y | RFC XXXX |
| 0x0004   | last_resort_key_package  | KP    | Y | RFC XXXX |
| 0x0005   | app_ack                  | AE    | Y | RFC XXXX |
| 0x0A0A   | GREASE                   | Note1 | Y | RFC XXXX |
| 0x1A1A   | GREASE                   | Note1 | Y | RFC XXXX |
| 0x2A2A   | GREASE                   | Note1 | Y | RFC XXXX |
| 0x3A3A   | GREASE                   | Note1 | Y | RFC XXXX |
| 0x4A4A   | GREASE                   | Note1 | Y | RFC XXXX |
| 0x5A5A   | GREASE                   | Note1 | Y | RFC XXXX |
| 0x6A6A   | GREASE                   | Note1 | Y | RFC XXXX |
...
The Safe Application Interface provides the following security guarantee: If an
...
- remove AppAck ProposalType as it is now a Component conveyed in AppEphemeral
  Proposals
