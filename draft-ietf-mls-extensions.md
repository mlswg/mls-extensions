...
IANA registry (see Section 17.3 of {{!RFC 9420}}).
...
This component of the Safe Extension API allows extensions to make use of a
number of HPKE key pairs created by MLS. An extension identified by an
ExtensionType can use any of these key pairs to transmit secret data as follows:

~~~ tls
SafeEncryptWithContext(ExtensionType, Context, Plaintext) =
EncryptWithLabel(PublicKey, “ExtensionData”, ExtensionContent, Plaintext)

SafeDecryptWithContext(ExtensionType, Context, Ciphertext) = 
DecryptWithLabel(SecretKey, “ExtensionData”, ExtensionContent, Ciphertext)
~~~

Where ExtensionContent (used as the info input to HPKE) includes the
ExtensionType of the implemented extension (and not any other extension), as
well as extension_data set to an arbitrary Context specified by the extension
designer (and can be empty if not needed).

In addition, an extension can use each HPKE key pair for any operation defined
in {{!RFC9180}}, such as exporting keys and the PSK mode, as long as the info
input is set to an ExtensionContent.

In general, a Ciphertext encrypted with PublicKey can be decrypted by any entity
who has the corresponding SecretKey at a given point in time according to the
MLS protocol (or extension). The key pairs generated by MLS include:

The key pair of a non-blank ratchet tree node. The SecretKey of such a key pair
is known to all members in the node’s subtree. In particular, a SecretKey of a
leaf node is known only to the member in that leaf. A member in the subtree
stores the SecretKey for a number of epochs, as long as the PublicKey does not
change. The key pair of the root node SHOULD NOT be used (see below).
...
The init_key in a KeyPackage and the corresponding secret key. The secret key is
known only to the owner of the KeyPackage and is deleted immediately after it is
...
respectively much like how the basic MLS protocol uses WithLabel and
VerifyWithLabel.

In more detail, extensions should sign and verify using:

~~~ tls
SafeSignWithLabel(ExtensionType, SignatureKey, Content) =
    SignWithLabel(SignatureKey, “ExtensionsDataTBS”, SignContent)

SafeVerifyWithLabel(ExtensionType, VerificationKey, Content, SignatureValue) =
    VerifyWithLabel(VerificationKey, “ExtensionDataTBS”, SignContent, SignatureValue)
~~~

Where SignContent is set to the serialized ExtensionContent struct (see Section
XX), where extension_data is set to the serialized SignContent struct specified
as follows (analogous as for SignWithLabel and VerifyWithLabel: in Section 5.1.2
of {{!RFC9420}}):
...
Where the fields are set as follows:
...
An extension can use MLS as a CGKA by exporting symmetric keys. Symmetric keys
can be exported (i.e. derived from protocol key material) in two phases per
...
4 of the MLS specification using the label `extension`.
...
DeriveExtensionSecret(Secret, Label) =
  ExpandWithLabel(Secret, “ExtensionExport ” + ExtensionID + ” “ + Label)
~~~

where ExpandWithLabel is defined in Section 8 of the MLS specification.

### Pre-Shared Keys (PSKs)

An extension can use MLS to enforce that all group members have access to
(potentially secret) information exchanged out of band, e.g. pre-shared secrets.
This is achieved by creating a new epoch via a PSK proposal. Transitioning to
the new epoch requires using the (secret) info agreed upon.

This document defines a new MLS ExtensionType in order to provide domain
separation between pre shared keys used by the core MLS protocol and
applications, and between those used by different extensions.
...
This document extends the IANA registries defined in the MLS specification as follows.

- MLS Wire Format (Section 17.2): mls_extension_message
- MLS Proposal Types (Section 17.4):
  - extension_proposal (requires no path and may not be sent by external senders)
